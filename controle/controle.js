...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== Header actions (com segurança contra null) =====
  ...
  const taskDiscipline = document.getElementById('taskDiscipline');
  ...
  function refreshDisciplines() {
    // garante 'Geral' no início e sem duplicar
    const arr = [...new Set(['Geral', ...state.disciplines.filter(d => d && d.trim())])];
    state.disciplines = arr;
    [disciplineSel, goalDiscipline, taskDiscipline, stopwatchDiscipline].forEach(sel => fillSelect(sel, arr));
  }
  refreshDisciplines();
  ...
  function tick() {
    ...
    if (remainingMs <= 0) {
      ...
      completeCycle();
      try { ding && ding.play && ding.play(); } catch (e) {}
    }
  }
  function start() {
    if (timer) return;
  ...
  swStartBtn && swStartBtn.addEventListener('click', () => {
    if (swRunning) return;
    swRunning = true; swStartAt = Date.now() - swElapsed;
    swTimer = setInterval(swTick, 200);
  });
  swPauseBtn && swPauseBtn.addEventListener('click', () => {
    if (!swRunning) return;
    swRunning = false; clearInterval(swTimer); swTimer = null; swTick();
  });
  swResetBtn && swResetBtn.addEventListener('click', () => {
    swRunning = false; clearInterval(swTimer); swTimer = null; swElapsed = 0; swLaps.length = 0;
    renderStopwatch(); renderLaps();
  });
  ...
  function minutesInWeekByDiscipline() {
    ...
    for (const s of state.sessions) {
      ...
      const t = s.ts;
      if (t >= monday.getTime() && t <= sunday.getTime()) acc[s.discipline] = (acc[s.discipline] || 0) + s.minutes;
    }
    return acc;
  }
  function updateGoalsView() {
    ...
    for (const d of state.disciplines) {
      ...
      const pct = goalMin ? Math.min(100, Math.floor((doneMin / goalMin) * 100)) : 0;
      ...
      wrap.innerHTML = `
        ...
        <div class="progress" style="margin:6px 0 12px"><span style="width:${pct}%"></span></div>
      `;
  ...
  function renderTasks() {
    if (!tasksList) return;
    tasksList.innerHTML = '';
    const tasks = [...state.tasks].sort((a, b) => (a.done - b.done) || (a.date || '').localeCompare(b.date || ''));
    for (const t of tasks) {
      const el = document.createElement('div'); el.className = 'task';
      const dateFmt = t.date ? new Date(t.date + 'T00:00:00').toLocaleDateString('pt-BR') : '-';
      el.innerHTML = `
        <div class="row" style="gap:10px">
          <input type="checkbox" ${t.done ? 'checked' : ''} data-id="${t.id}" class="tchk"/>
          <div>
            <div><strong>${t.title}</strong> <span class="tag">${t.discipline}</span></div>
            <small style="color:#666">Entrega: ${dateFmt}</small>
          </div>
        </div>
        <button class="btn ghost" data-rm="${t.id}"><i class="fa-solid fa-xmark"></i></button>
      `;
      tasksList.appendChild(el);
    }
    tasksList.querySelectorAll('.tchk').forEach(i => i.addEventListener('change', e => {
      const id = e.target.getAttribute('data-id');
      const t = state.tasks.find(x => x.id === id);
      if (t) { t.done = e.target.checked; save(); renderTasks(); }
    }));
    tasksList.querySelectorAll('[data-rm]').forEach(btn => btn.addEventListener('click', () => {
      const id = btn.getAttribute('data-rm');
      state.tasks = state.tasks.filter(x => x.id !== id); save(); renderTasks();
    }));
  }
  addTaskBtn && addTaskBtn.addEventListener('click', () => {
    const title = (taskTitle && taskTitle.value || '').trim(); if (!title) return;
    const d = taskDiscipline && taskDiscipline.value || 'Geral';
    const date = (taskDate && taskDate.value) || '';
    state.tasks.push({ id: crypto.randomUUID(), title, date, discipline: d, done: false });
    save();
  ...
  // ===== Gráficos (tolerante se Chart.js não carregar) =====
  ...
  function hoursLastNDays(n = 14) {
    ...
    for (let i = n - 1; i >= 0; i--) {
      ...
      const m = state.sessions
        .filter(s => s.mode === 'focus' && s.ts >= start && s.ts <= end)
  ...
  function distributionByDiscipline() {
    const by = {};
    for (const s of state.sessions) { if (s.mode === 'focus') by[s.discipline] = (by[s.discipline] || 0) + s.minutes; }
    const labels = Object.keys(by);
    const hours = labels.map(k => Math.round(by[k] / 60 * 100) / 100);
    return { labels, hours };
  }
  function updateCharts() {
    if (typeof Chart === 'undefined' || !lineCtx || !pieCtx) return; // se CDN falhar
    ...
    try {
      lineChart = new Chart(lineCtx, {
        type: 'line',
        data: { labels: hl.labels, datasets: [{ label: 'Horas por dia', data: hl.hours, tension: .3 }] },
        options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
      });
      pieChart = new Chart(pieCtx, {
        type: 'doughnut',
        data: { labels: dist.labels, datasets: [{ data: dist.hours }] },
  ...
  // ===== Init =====
  renderTasks(); updateHistory(); updateCharts(); updateGoalsView();
