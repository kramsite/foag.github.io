...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .feature-category{margin:18px 0 8px; font-weight:800; color:var(--brand-700); display:flex; align-items:center; gap:8px}
    .feature-category .tag{font-size:12px; padding:4px 8px; border:1px solid #dbeafe; background:#eff6ff; color:#1d4ed8; border-radius:999px}
    .features-grid{display:grid; grid-template-columns:repeat(2, minmax(0,1fr)); gap:16px}
    .feature-card{background:var(--surface); border:1px solid #ddd; border-radius:16px; padding:18px; display:grid; grid-template-columns:auto 1fr; gap:14px; align-items:flex-start; box-shadow:0 6px 16px rgba(0,0,0,.04)}
    .feature-card .big-icon{width:46px; height:46px; border-radius:14px; display:grid; place-items:center; background:#e6f2ff; color:var(--brand-600); font-size:22px}
    .feature-card h3{margin:2px 0 6px}
    .feature-card p{margin:0; color:var(--muted)}
    .feature-card .actions{margin-top:10px; display:flex; gap:10px; flex-wrap:wrap}
    .feature-card .link{display:inline-flex; align-items:center; gap:8px; padding:8px 12px; border-radius:10px; text-decoration:none; background:#fff; border:1px solid #dbeafe; color:#1d4ed8; font-weight:600}
    .feature-card .link:hover{background:#f8fbff}
...
<body>
  ...
  <main>
    ...
    <!-- Recursos extras -->
  <section id="recursos">
    <h2 class="title">Recursos do FOAG</h2>
    <p class="sub">Tudo que você precisa, sem complicação. Veja o que o FOAG oferece:</p>
    <div class="features-grid">
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-calendar-days"></i></div>
        <div>
          <h3>Horários semanais</h3>
          <p>Monte e visualize seu quadro de horários de forma simples.</p>
        </div>
      </article>
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-user-check"></i></div>
        <div>
          <h3>Presenças e faltas</h3>
          <p>Registre e acompanhe presenças e faltas em cada disciplina.</p>
        </div>
      </article>
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-clipboard"></i></div>
        <div>
          <h3>Anotações por aula</h3>
          <p>Guarde resumos e anotações rápidas em cada matéria.</p>
        </div>
      </article>
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-square-poll-vertical"></i></div>
        <div>
          <h3>Boletim e médias</h3>
          <p>Lance notas, pesos e veja sua média parcial e final.</p>
        </div>
      </article>
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-stopwatch"></i></div>
        <div>
          <h3>Timer de estudo</h3>
          <p>Controle sessões de foco e acompanhe seu histórico.</p>
        </div>
      </article>
      <article class="feature-card">
        <div class="big-icon"><i class="fa-solid fa-rotate"></i></div>
        <div>
          <h3>Revisão espaçada</h3>
          <p>Receba lembretes automáticos para revisar no tempo certo.</p>
        </div>
      </article>
    </div>
  </section>
